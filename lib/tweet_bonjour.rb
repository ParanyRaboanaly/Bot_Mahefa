...
#  puts "#{select_client}"
  ...
  config.access_token        = ENV["TWITTER_ACCESS_TOKEN"]
  config.access_token_secret = ENV["TWITTER_ACCESS_TOKEN_SECRET"]
  client.follow(select_client).each do |tweet|
  client.update("@#{tweet.user} Bonjour..voici mon premier tweet en ruby :)")
  end
end

end
#puts random_items(array)
#client.search(select_client).take(5)
